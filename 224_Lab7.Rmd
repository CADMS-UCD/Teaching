...
par(mfrow = c(2,2))
# method, ignore: ignore values ot of bounds
truncateDist(x = x, range = c(10, 90)) %>% hist(main = 'Ignore')
# method substitute: substitute values out of bounds for min max provided
truncateDist(x = x, range = c(10, 90), method = 'sub') %>% hist(main = 'Substitution')
# Method resample: Resample values out of bounds for values inside bounds
truncateDist(x = x, range = c(10, 90), method = 'resample') %>% hist(main = 'Resample')
```
...
M <- data.frame(
  ...
  formula = c(NA, NA, 'Insert formula here', # O1 Contact
